...
import { EXAM_CONFIG } from "@/lib/ap-physics-1/config";

describe("AP Physics 1 Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM175");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("ap-physics-1");
  });
